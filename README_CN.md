...
#### Claude Code Proxy（代理）

使用本地 [claude-code-proxy](https://github.com/nicekid1/Claude-Code-Proxy) 通过 Claude MAX 订阅的 OAuth 令牌转发请求——无需 API 密钥。

1. 安装并启动代理（`npm start`——默认运行在 `http://localhost:42069`）
2. 打开 **设置** > **AI** 标签页
3. 选择 **Claude Code Proxy** 作为服务商
4. （可选）如果代理运行在其他端口，调整代理基础 URL
5. （可选）选择模型（Sonnet 4.5、Sonnet 4、Opus 4 或 Haiku 3.5）

代理暴露标准的 Anthropic Messages API，支持 SSE 流式传输。无需 API 密钥——代理透明地处理 OAuth 认证。
...
5. 确保在 AI 标签页中配置了 AI 服务商（Claude、Gemini 或 Proxy）
...
| 设置 | 默认值 | 说明 |
|------|--------|------|
| AI 服务商 | Claude | 在 Claude（Anthropic）、Gemini（Google）或 Claude Code Proxy 之间选择 |
| Claude API 密钥 | （空） | 你的 Anthropic API 密钥 |
| Gemini API 密钥 | （空） | 你的 Google AI Studio API 密钥 |
| 代理基础 URL | `http://localhost:42069` | Claude Code Proxy 的地址（仅选择 Proxy 时显示） |
| 代理模型 | Claude Sonnet 4.5 | 通过代理使用的模型：Sonnet 4.5、Sonnet 4、Opus 4 或 Haiku 3.5 |

API 密钥保存在本地 `config.json` 中。代理模式无需 API 密钥——代理自行处理认证。没有密钥或代理时聊天回退到离线脚本回复。
...
│   │   ├── ProxyApiService.cs      # Claude Code Proxy API（无需 API 密钥）
...
│       └── Seal/                   # 海豹变身素材
│
├── tests/AemeathDesktopPet.Tests/  # 791 个测试，42+ 个测试类
│   ├── Models/                    # 10 个测试类
│   ├── Engine/                    # 8 个测试类
│   ├── Services/                  # 21 个测试类（单元 + 集成 + E2E）
│   ├── ViewModels/                # 2 个测试类
│   └── Interop/                   # 1 个测试类
│
...
- **xUnit** — 791 个 C# 测试 + 157 个 Python 测试，42+ 个测试类（单元、集成、E2E）
