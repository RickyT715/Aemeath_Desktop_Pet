...
/// <summary>
/// Claude Code Proxy chat service — routes through a local proxy that handles OAuth.
/// No API key needed; the proxy authenticates via Claude MAX subscription tokens.
/// Falls back to OfflineResponses when the proxy URL is not configured.
/// </summary>
public class ProxyApiService : IChatService
{
    ...
    public ProxyApiService(Func<AppConfig> getConfig, Func<AemeathStats> getStats)
    {
        ...
        {
            BaseAddress = new Uri(string.IsNullOrWhiteSpace(config.ProxyBaseUrl)
    ...
    public bool IsAvailable => !string.IsNullOrWhiteSpace(_getConfig().ProxyBaseUrl);
    ...
    private string BuildPayload(string userMessage, IReadOnlyList<ChatMessage> history, bool stream, byte[]? screenshot = null)
    {
        ...
        var model = string.IsNullOrWhiteSpace(config.ProxyModel)
